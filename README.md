
<html lang="hi">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>AcademeForge - Coming Soon</title>
  <style>
    body {
      margin: 0;
      padding: 0;
      overflow: hidden;
      height: 100vh;
      background: black;
      display: flex;
      justify-content: center;
      align-items: center;
      font-family: Arial, sans-serif;
      color: white;
    }

    /* Matrix rain background */
    canvas {
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      z-index: -1;
      background: black;
    }

    .card {
      width: 340px;
      height: 460px;
      perspective: 1000px;
    }
    .card-inner {
      position: relative;
      width: 100%;
      height: 100%;
      transform-style: preserve-3d;
      transition: transform 1s;
      cursor: pointer;
    }
    .card.flipped .card-inner {
      transform: rotateY(180deg);
    }
    .card-front, .card-back {
      position: absolute;
      width: 100%;
      height: 100%;
      backface-visibility: hidden;
      border-radius: 16px;
      display: flex;
      flex-direction: column;
      justify-content: center;
      align-items: center;
      padding: 20px;
      background: rgba(20, 20, 20, 0.9);
      box-shadow: 0 0 20px rgba(0,255,128,0.4);
      text-align: center;
    }
    .card-front h1 { font-size: 2rem; margin-bottom: 10px; color: #0ff; }
    .card-front h2 { font-size: 1.3rem; color: #0f0; }
    .card-front p { margin-top: 10px; color: #ccc; }

    .card-back {
      transform: rotateY(180deg);
      background: rgba(15, 15, 15, 0.95);
      box-shadow: 0 0 20px rgba(0,200,255,0.4);
    }
    .card-back h1 { font-size: 1.6rem; margin-bottom: 10px; color: #0ff; }
    .card-back h3 { color:#0f9; margin: 10px 0; }

    form { display: flex; gap: 10px; margin: 15px 0; flex-wrap: wrap; justify-content: center; }
    input[type="email"] {
      padding: 10px;
      border: none;
      border-radius: 6px;
      background: rgba(255,255,255,0.1);
      color: white;
      min-width: 180px;
    }
    button {
      padding: 10px 16px;
      border: none;
      border-radius: 6px;
      background: linear-gradient(90deg,#06f,#0f9);
      color: white;
      font-weight: bold;
      cursor: pointer;
    }
    .links a {
      display: block;
      margin: 5px 0;
      color: #0ff;
      text-decoration: none;
    }
    .countdown {
      display: flex;
      gap: 14px;
      margin-top: 15px;
    }
    .time-box {
      background: rgba(255,255,255,0.08);
      padding: 10px;
      border-radius: 8px;
      font-family: monospace;
      font-size: 1rem;
      font-weight: 600;
    }
    .label { font-size: 0.7rem; color: #bbb; margin-top: 4px; }
  </style>
</head>
<body>
  <canvas id="matrix"></canvas>

  <div class="card" id="flipCard">
    <div class="card-inner">
      <!-- Front in Hindi -->
      <div class="card-front">
        <h1>अकैडमीफोर्ज</h1>
        <h2>जल्द ही आ रहा है 🚀</h2>
        <p>शिक्षा और तकनीक का संगम</p>
        <p style="margin-top:20px;color:#0ff;font-size:0.85rem;">(फ्लिप करने के लिए क्लिक करें)</p>
      </div>

      <!-- Back in Hindi -->
      <div class="card-back">
        <h1>AcademeForge</h1>
        <p>We are building the future of learning and innovation.</p>
        <h3>अपडेट प्राप्त करें</h3>
        <form onsubmit="event.preventDefault()">
          <input type="email" placeholder="अपना ईमेल दर्ज करें" required />
          <button type="submit">मुझे सूचित करें</button>
        </form>
        <div class="links">
          <a href="#">LinkedIn पर हमसे जुड़ें</a>
          <a href="#">Twitter पर हमारा अनुसरण करें</a>
        </div>
        <div class="countdown">
          <div><div class="time-box" id="days">00</div><div class="label">दिन</div></div>
          <div><div class="time-box" id="hours">00</div><div class="label">घंटे</div></div>
          <div><div class="time-box" id="minutes">00</div><div class="label">मिनट</div></div>
          <div><div class="time-box" id="seconds">00</div><div class="label">सेकंड</div></div>
        </div>
      </div>
    </div>
  </div>

  <script>
    // Flip card
    document.getElementById('flipCard').addEventListener('click', () => {
      document.getElementById('flipCard').classList.toggle('flipped');
    });

    // Countdown
    const targetDate = new Date();
    targetDate.setDate(targetDate.getDate() + 30);
    function updateCountdown() {
      const now = new Date().getTime();
      const distance = targetDate - now;
      const days = Math.floor(distance / (1000 * 60 * 60 * 24));
      const hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
      const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
      const seconds = Math.floor((distance % (1000 * 60)) / 1000);
      document.getElementById('days').textContent = days;
      document.getElementById('hours').textContent = hours;
      document.getElementById('minutes').textContent = minutes;
      document.getElementById('seconds').textContent = seconds;
    }
    setInterval(updateCountdown, 1000);
    updateCountdown();

    // Matrix rain background
    const canvas = document.getElementById("matrix");
    const ctx = canvas.getContext("2d");
    canvas.height = window.innerHeight;
    canvas.width = window.innerWidth;

    const letters = "01ABCDEFGHIJKLMNOPQRSTUVWXYZअआइईउऊएऐओऔकखगघचछजझटठडढतथदधनपफबभमयरलवशषसह";
    const matrix = letters.split("");
    const font_size = 14;
    const columns = canvas.width / font_size;
    const drops = [];
    for (let x = 0; x < columns; x++) drops[x] = 1;

    function draw() {
      ctx.fillStyle = "rgba(0, 0, 0, 0.05)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.fillStyle = "#0f0";
      ctx.font = font_size + "px monospace";
      for (let i = 0; i < drops.length; i++) {
        const text = matrix[Math.floor(Math.random() * matrix.length)];
        ctx.fillText(text, i * font_size, drops[i] * font_size);
        if (drops[i] * font_size > canvas.height && Math.random() > 0.975) drops[i] = 0;
        drops[i]++;
      }
    }
    setInterval(draw, 33);
  </script>
</body>
</html>